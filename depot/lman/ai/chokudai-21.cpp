...
int checkdfs(int y, int x, int depth, int predirection, int count){
	int ret = 0;
	if(getNumber(y, x) > 0){
		if(toi(read_tree2d(distghosttree, y, x)) > addnum){
			ret = 1;
		}
		else{
			if(vitality > 0){
...
void bfs(){
	...
	while(1 - is_empty_queue(q1)){
		...
		mypoint = toi(read_tree2d(distlmantree, y, x)) + 1000000;
		...
		else
		{
			...
			if(cost < vitality * 5 / 3){
				if(d == 2){}
					...
					num = getNumber(y-1, x);
					point = mypoint + getPoint2(num) * pow2(10 - cost) * 100;
					if(num==3){
						...
						if(num == 0){}
							...
							if(toi(read_tree2d(distlmantree, y-1, x)) == MAXNUM){
								if(toi(read_tree2d(distghosttree, y-1, x)) == 0){
									point = point + 50000 * pow2(10-cost);
								}

								distlmantree = write_tree2d(distlmantree, y-1, x, point);
					...
					if(num==3){
						...
						if(num == 0){}
							...
							if(toi(read_tree2d(distlmantree, y, x+1)) == MAXNUM){
								if(toi(read_tree2d(distghosttree, y, x + 1)) == 0){
									point = point + 50000 * pow2(10-cost);
								}
								distlmantree = write_tree2d(distlmantree, y, x+1, point);
				...
				if(d == 2){}
					...
					point = mypoint + getPoint2(num) * pow2(10 - cost) * 100;
					if(num==3){
						gdist = toi(read_tree2d(distghosttree, y-1,x)) - cost - addnum;
						if(gdist > 0){
							if(gdist < 3){
								point = point + 1300000000;
							}
						}
						else{
							point = point + 300000000;
						...
						if(num == 0){}
							...
							if(toi(read_tree2d(distlmantree, y-1, x)) == MAXNUM){
								if(toi(read_tree2d(distghosttree, y-1, x)) > cost + addnum){
									distlmantree = write_tree2d(distlmantree, y-1, x, point);
									q1 = push_queue(q1, P(P(x, y - 1), P(0, P(cost+1, id))));
				...
				if(d == 3){}
					...
					point = mypoint + getPoint2(num) * pow2(10 - cost)* 100;
					if(num==3){
						gdist = toi(read_tree2d(distghosttree, y,x + 1)) - cost - addnum;
						if(gdist > 0){
							if(gdist < 3){
								point = point + 1300000000;
							}
						...
						if(num == 0){}
							...
							if(toi(read_tree2d(distlmantree, y, x+1)) == MAXNUM){
								if(toi(read_tree2d(distghosttree, y, x + 1)) > cost + addnum){ 
									distlmantree = write_tree2d(distlmantree, y, x+1, point);
									q1 = push_queue(q1, P(P(x+1, y),  P(1, P(cost+1, id))));
...
void bfs2(){
	...
	if(bfsgnum == 0){ uppoint -= 20000000; }
		...
		if(checkdfs(ly-1,lx,10,0,1)){
			if(bfsgnum==3){
				...
				point = getPoint(ly-1, lx) * pow2(10) * 100;
				distlmantree = write_tree2d(distlmantree, ly-1, lx, point);
				q1 = push_queue(q1, P(P(lx,ly-1), P(-1, P(1, 0))));
				uppoint = mymax(uppoint, point);
				bfs();
			}
		}
	...
	if(getNumber(ly,lx+1) == 0){ rightpoint -= 20000000; }
	else{
		if(checkdfs(ly,lx + 1,10,0,1)){
			if(bfsgnum==3){
				...
				distlmantree = allmaxtree;
				point = getPoint(ly, lx+1) * pow2(10) * 100;
				distlmantree = write_tree2d(distlmantree, ly, lx+1, point);
	...
	if(getNumber(ly+1,lx) == 0){ downpoint -= 20000000; }
	else{
		if(checkdfs(ly+1,lx,10,0,1)){
			if(bfsgnum==3){
				uppoint = mymax(uppoint, 1300000000);
				...
				point = getPoint(ly+1, lx) * pow2(10) * 100;
				distlmantree = write_tree2d(distlmantree, ly+1, lx, point);
				q1 = push_queue(q1, P(P(lx,ly+1), P(-1, P(1, 2))));
				downpoint = mymax(downpoint, point);
				bfs();
			}
		}
	...
	if(getNumber(ly,lx-1) == 0){ leftpoint -= 20000000; }
	else{
		if(checkdfs(ly,lx-1,10,0,1)){
			if(bfsgnum==3){
				uppoint = mymax(uppoint, 1300000000);
				...
				point = getPoint(ly, lx-1) * pow2(10) * 100;
				distlmantree = write_tree2d(distlmantree, ly, lx-1, point);
				q1 = push_queue(q1, P(P(lx-1,ly), P(-1, P(1, 3))));
				leftpoint = mymax(leftpoint, point);
				bfs();
			}
		}
...
P step2(P ai, P game) {
	...
	vitality = toi(fst(lambdaman)) / 137 - 1;
	...
	addnum = 2;
	if(toi(read_tree2d(distghosttree, ly,lx)) < 2){
		addnum = 1;
	}
