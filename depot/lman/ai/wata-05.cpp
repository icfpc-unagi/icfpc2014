...
int evalAround(int x, int y, int near) {
	...
	P canReachLambda = set_array2d32(list2d_to_array2d32(createList2D(D * 2 + 1, D * 2 + 1, -1)), D, D, 0); // -1: unreachable, >=0, distance
	P canReachLambda2;
	P canReachGhost = list2d_to_array2d32(createList2D(D * 2 + 1, D * 2 + 1, -1)); // -1: unreachable, 0-3: one direction, 4: any directions
	P canReachGhost2;
	while (atom(ghosts) == 0) {
		ghost = fst(ghosts);
		ghosts = snd(ghosts);
	...
	t = 0;
	while (t < D) {
		canReachGhost2 = list2d_to_array2d32(createList2D(2 * D + 1, 2 * D + 1, -1));
		i = 0;
		while (i <= 2 * D) {
			j = 0;
			while (j <= 2 * D) {
						...
						if (mymod(d + 2, 4) == k) {
							...
							if (0 <= i2) {
								if (i2 <= 2 * D) {
									if (0 <= j2) {
										if (j2 <= 2 * D) {
											if (toi(get_array2d32(subBoard, j2, i2)) > 0) {
		...
		i = 0;
		canReachLambda2 = list2d_to_array2d32(createList2D(2 * D + 1, 2 * D + 1, -1));
		live = 0;
		while (i <= 2 * D) {
			j = 0;
			while (j <= 2 * D) {
										...
										if (0 <= j2) {
											if (j2 <= 2 * D) {
												if (toi(get_array2d32(canReachGhost, j2, i2)) < 0) {
													c = toi(get_array2d32(subBoard, j2, i2));
													if (c > 0) {
														if (c == 3) {
															takePowerPill = 1;
		...
		while (j <= 2 * D) {
			if ((i == D) * (j == D)) {
				...
				if (d >= 0) {
					...
					k = toi(get_array2d32(subBoard, j, i));
					if (k == 2) {
						ret += 1000 / (d + 1);
					} else {
						if (k == 3) {
