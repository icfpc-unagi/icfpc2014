...
void bfs(){
	...
	while(1 - is_empty_queue(q1)){
		...
		mypoint = toi(get_array2d256(distlmantree, y, x));
		if(turn < wideturn){
			mypoint = mypoint + 1000000;
		}
						...
						if(num == 0){}
							...
							if(toi(get_array2d256(distlmantree, y-1, x)) == MAXNUM){
								if(toi(get_array2d256(distghosttree, y-1, x)) == 0){
									point = point + 50001 * pow2(10-cost);
								}
...
P step2(P ai, P game) {
	...
	if(turn > wideturn * 10){
		temp = mod(turn - wideturn, h * w * 2);
		if(temp < h * w){
			targety = temp / w / 5;
			targetx = (temp - targety * w * 5) / 25;
		...
		uppoint = uppoint - (ly - targety) / 5 * 10000;
		rightpoint = rightpoint + (lx - targety) / 5 * 10000;
		downpoint = downpoint + (ly - targety) / 5 * 10000;
		leftpoint = leftpoint - (lx - targety) / 5 * 10000;
	}
	else{
		if(randomv == 0){
