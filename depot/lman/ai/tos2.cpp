...
//  }
  ...
  i=0; while (i<imax) {
	...
	if (fruit_time > 0) {  // there exists a fruit now
	  ...
	  u = t - 2*u;  // dorekurai toresou?  2bai ha jikan mitumotte oku.
	  if (u < 0) {
	    u = 0;
	  }
	  s = s * u / (t + 1);
...
//	  s = s * fruit_time / (127*80);
  ...
  // smell = write_tree2d(smell, li,lj, toi(read_tree2d(smell, li, lj))/3);  // I've just visited.
