...
void test_array3(int n) {
  ...
  while (i < n) {
    j = 0;
    while (j < n) {
      a = set_array2d32(a, i, j, (i + 1) * 1000 + (j + 1));
      j += 1;
    }
    i += 1;
  }
  ...
  while (i < n) {
    j = 0;
    while (j < n) {
      debug(P(P(i, j), get_array2d32(a, i, j)));
      j += 1;
    }
    i += 1;
  }
