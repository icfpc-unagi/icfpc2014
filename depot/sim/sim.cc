...
constexpr int kFruitPoints[14] = {0, 100, 300, 500, 500, 700, 700, 1000, 1000, 2000, 2000, 3000, 3000, 5000};
...
void Game::ParseMaze(std::istream& is) {
    ...
    while (true) {
        ...
        CHECK(maze_.empty() || maze_.back().size() != line.size()) << "Input maze format error";
        for (int i = 0; i < line.size(); ++i) {
            switch (line[i]) {
                case '\\':  // Lambda-Man
                    ...
                    break;
                case '=':  // Ghost
                    ghosts_.emplace_back(ghost_factories_[ghost_index % ghost_factories_.size()]->Create());
    ...
    LOG_IF(FATAL, fruit_locations_.size() > kFruits) << "# of fruits must be " << kFruits;
    LOG_IF(WARNING, fruit_locations_.size() < kFruits) << "# of fruits must be " << kFruits;
}
...
int Game::Start() {
    ...
    for (int i = 0; i < ghosts_.size(); ++i) utc_ghosts_next_moves[i] = 130 + 2 * i;
        ...
        for (int i = 0; i < ghosts_.size(); ++i) {
            if (!ghosts_invisible_[i] && ghosts_[i]->GetRC() == pos) {
                if (flight_mode == 0) {
                    ...
                    // Loses a life
                    life_--;
                    // Resets Lambda-Man and all ghosts returned to starting positions and directions
